...
case "$PACK" in
  C) SRC="$ROOT/machine/targetC/server"; SRVMARK="forge-1.12.2-14.23.5.2846-universal.jar"
     PORT=25567; BOTUSER=perfbotb; LABEL=m58rev-shadow; MODLIST="$ROOT/machine/targetC/mod-ids.json" ;;
...
( sleep 90
  echo "say M56 resend rig: reconnect sessions"
  sleep 480
  echo "stop"
  sleep 60
) | "$JAVA" -Xmx6G -Xms6G -Djava.library.path=. \
  ...
  -Dminecraftrust.native_compress=SHADOW \
  -Dminecraftrust.m58.canary=true \
...
( M1_MODLIST="$MODLIST" M1_USER=$BOTUSER \
    M1_TP=30 M1_TP_INTERVAL=2 M1_TP_STEP=180 M1_TP_Z_STEP=90 M1_TP_SPAN=1440 \
    HOLD_SECONDS=45 SESSIONS=8 \
...
cp m1-metrics-postcleanup.txt "$ROOT/machine/raw/$LABEL-postcleanup.txt" 2>/dev/null || true
