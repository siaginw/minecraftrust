#!/usr/bin/env bash
# M3 Worldgen Target-A Authoritative HARDENING campaign.
# Twin-world Pre-Population Terrain Oracle + live timing characterization.
# Runs:
#   World J: M3 worldgen OFF_MEASURE (pure Java reference)  -> m3wg-prepop-OFF_MEASURE.txt
#   World R: M3 worldgen ON_EXPERIMENTAL (Rust authoritative) -> m3wg-prepop-ON_EXPERIMENTAL.txt
...
mkdir -p "$ROOT/machine/raw/hardening"

run_world() {
    local WORLD_TAG="$1"     # "worldj" or "worldr"
    local WG_MODE="$2"       # "OFF_MEASURE" or "ON_EXPERIMENTAL"
    local RUN_DIR="$ROOT/machine/targetA/run-$WORLD_TAG-h"
    local LOG_FILE="$RUN_DIR/server.log"
    local CLIENT_LABEL="M3WG-hard-$WORLD_TAG"
...
EOF
    ...
    for i in $(seq 1 120); do
        ...
        if ! kill -0 "$SRV_PID" 2>/dev/null; then
            echo "SERVER PROCESS DIED EARLY!"; cat "$LOG_FILE"; exit 1
        fi
    done
    ...
    TARGETA_OUT_DIR="$ROOT/machine/raw/hardening" \
    ...
    # Harvest artifacts
    cp "$RUN_DIR/m1-metrics.txt" "$ROOT/machine/raw/hardening/$TAG-$WORLD_TAG-metrics.txt" 2>/dev/null || true
    cp "$LOG_FILE" "$ROOT/machine/raw/hardening/$TAG-$WORLD_TAG-server.log" 2>/dev/null || true
    if [ -f "$ROOT/machine/raw/hardening/client-$CLIENT_LABEL.jsonl" ]; then
        mv "$ROOT/machine/raw/hardening/client-$CLIENT_LABEL.jsonl" \
           "$ROOT/machine/raw/hardening/$TAG-$WORLD_TAG-client.jsonl"
    fi
    cp "$RUN_DIR/m3wg-prepop-$WG_MODE.txt" "$ROOT/machine/raw/hardening/$TAG-$WORLD_TAG-prepop.txt" 2>/dev/null \
        && echo "Harvested prepop: $TAG-$WORLD_TAG-prepop.txt" || echo "WARN: prepop file missing for $WORLD_TAG"

    # Preserve generated world (secondary post-population evidence)
    rm -rf "$ROOT/machine/targetA/world-$WORLD_TAG-h"
    cp -r "$RUN_DIR/world" "$ROOT/machine/targetA/world-$WORLD_TAG-h"
    echo "Saved world to $ROOT/machine/targetA/world-$WORLD_TAG-h"
}
...
echo "=================================================================="
echo "COMPARING PRE-POPULATION TERRAIN ORACLES (JAVA vs RUST)"
echo "=================================================================="
python "$ROOT/tools/compare_prepop.py" \
  "$ROOT/machine/raw/hardening/$TAG-worldj-prepop.txt" \
  "$ROOT/machine/raw/hardening/$TAG-worldr-prepop.txt" \
  "$ROOT/machine/raw/hardening/$TAG-prepop-comparison.txt" \
  || echo "(compare_prepop exited nonzero: see $TAG-prepop-comparison.txt)"

echo "=================================================================="
echo "RECOMPARING POST-POPULATION WORLDS (secondary evidence)"
echo "=================================================================="
python "$ROOT/tools/compare_worlds.py" \
  "$ROOT/machine/targetA/world-worldj-h" \
  "$ROOT/machine/targetA/world-worldr-h" \
  | tee "$ROOT/machine/raw/hardening/$TAG-postpop-comparison.txt"

echo "HARDENING CAMPAIGN COMPLETE."
