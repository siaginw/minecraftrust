#!/usr/bin/env bash
# M3 Worldgen AUTHORITATIVE campaign: FTB Revelation 3.4.0 (Target C)
...
mkdir -p "$ROOT/machine/raw/rev-auth"

run_world() {
    local WORLD_TAG="$1"     # "worldj" or "worldr"
    local WG_MODE="$2"       # "OFF_MEASURE" or "ON_EXPERIMENTAL"
    local RUN_DIR="$ROOT/machine/targetC/run-$WORLD_TAG-auth"
    local LOG_FILE="$RUN_DIR/server.log"
    local CLIENT_LABEL="M3WG-rev-auth-$WORLD_TAG"

    echo "=================================================================="
    echo "STARTING RUN: $WORLD_TAG (Mode: $WG_MODE) — Revelation"
    echo "=================================================================="

    rm -rf "$RUN_DIR"
    mkdir -p "$RUN_DIR"

    # Copy server assets (excluding world, logs)
...
EOF
    ...
    TARGETA_OUT_DIR="$ROOT/machine/raw/rev-auth" \
    ...
    # Harvest artifacts
    cp "$RUN_DIR/m1-metrics.txt" "$ROOT/machine/raw/rev-auth/$TAG-$WORLD_TAG-metrics.txt" 2>/dev/null || true
    cp "$LOG_FILE" "$ROOT/machine/raw/rev-auth/$TAG-$WORLD_TAG-server.log" 2>/dev/null || true
    if [ -f "$ROOT/machine/raw/rev-auth/client-$CLIENT_LABEL.jsonl" ]; then
        mv "$ROOT/machine/raw/rev-auth/client-$CLIENT_LABEL.jsonl" \
           "$ROOT/machine/raw/rev-auth/$TAG-$WORLD_TAG-client.jsonl"
    fi
    cp "$RUN_DIR/m3wg-prepop-$WG_MODE.txt" "$ROOT/machine/raw/rev-auth/$TAG-$WORLD_TAG-prepop.txt" 2>/dev/null \
...
echo "=================================================================="
echo "COMPARING PRE-POPULATION TERRAIN ORACLES (JAVA vs RUST) — Revelation"
echo "=================================================================="
python "$ROOT/tools/compare_prepop.py" \
  "$ROOT/machine/raw/rev-auth/$TAG-worldj-prepop.txt" \
  "$ROOT/machine/raw/rev-auth/$TAG-worldr-prepop.txt" \
  "$ROOT/machine/raw/rev-auth/$TAG-prepop-comparison.txt" \
  || echo "(compare_prepop exited nonzero: see $TAG-prepop-comparison.txt)"

echo "REVELATION AUTHORITATIVE CAMPAIGN COMPLETE."
