#!/usr/bin/env bash
# M4 NativeChunk Foundation LIVE SHADOW run (clean Forge Target A).
# Target A ONLY, DISPOSABLE COPY of the tracked world (tracked fixtures untouched).
# Java authoritative; Rust shadow retains NativeChunk, validates base terrain bit-identity,
# and exercises zero-copy packet encoding + occupancy + persistence staging.
set -u
...
( M1_MODLIST="$ROOT/machine/targetA/mod-ids.json" M1_USER=m2cebot \
    ...
    python "$ROOT/tools/targeta_client.py" m4-shadow 127.0.0.1 25565 >/dev/null 2>&1 || true ) &
wait $SRV || true
cp m1-metrics.txt "$ROOT/machine/raw/M4-shadow-metrics-final.txt" 2>/dev/null
echo "M4 SHADOW RUN COMPLETE"
