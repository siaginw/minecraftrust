...
public class ProbeNativeNull2 {
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("before: FALLBACKS=" + NativeChunkPacket.M1_NATIVE_FALLBACKS.get()
                + " PANICS=" + NativeChunkPacket.M1_NATIVE_PANICS.get()
                + " INELIGIBLE=" + NativeChunkPacket.M1_NATIVE_INELIGIBLE.get()
                + " TRANSMITTED=" + NativeChunkPacket.M1_NATIVE_PACKETS_TRANSMITTED.get());
        try {
            boolean ok = NativeChunkPacket.populatePacket(new net.minecraft.network.play.server.SPacketChunkData(), c, 0x0007);
            System.out.println("returned " + ok);
        } catch (Throwable t) {
            System.out.println("THREW " + t.getClass().getName() + " at " + t.getStackTrace()[0]);
        }
        System.out.println("after : FALLBACKS=" + NativeChunkPacket.M1_NATIVE_FALLBACKS.get()
                + " PANICS=" + NativeChunkPacket.M1_NATIVE_PANICS.get()
                + " INELIGIBLE=" + NativeChunkPacket.M1_NATIVE_INELIGIBLE.get()
                + " TRANSMITTED=" + NativeChunkPacket.M1_NATIVE_PACKETS_TRANSMITTED.get());
    }
